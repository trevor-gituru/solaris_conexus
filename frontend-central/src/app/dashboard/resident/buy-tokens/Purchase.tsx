...
useEffect(() => {
  const fetchTransactions = async () => {
    try {
      ...
      showToast('An error occurred while fetching transactions', { type: 'error' });
      setTransactions([]);
    } finally {
      setIsFetching(false);
    }
...
const handleBuyStrk = async () => {
  ...
  try {
    ...
    const bodyPayload = {
	  ...
	  strk_used: strk_used,
	  payment_method: 'STRK', // or 'MPESA' depending on logic
	  date: new Date().toISOString(),  // current date/time in ISO format
   };

    // POST new purchase to backend
    const addRes = await fetch(`${API_URL}/residents/token_purchase/add_strk`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
    ...
    showToast(err?.message || 'An error occurred during purchase', { type: 'error' });
  } finally {
    setIsLoading(false);
  }
};
...
const handleMpesaInit = async () => {
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/residents/token_purchase/add_mpesa`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
    ...
    if (!response.ok) {
      const errMsg = result?.detail || result?.message || 'Failed to initiate MPESA payment';
      showToast(errMsg, { type: 'error' });
      setIsLoading(false);
      return;
    }

    showToast(result?.message || 'M-PESA payment recorded successfully!', { type: 'success' });
    ...
    showToast(error?.message || 'Unexpected error occurred during M-PESA payment.', {
      type: 'error',
    });
  } finally {
    setIsLoading(false);
...
const handleMpesaConfirm = async () => {
  ...
  try {
    ...
    const confirmRes = await fetch(`${API_URL}/residents/token_purchase/confirm_mpesa`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${token}`,
      },
    ...
    if (!confirmRes.ok) {
      const errMsg = confirmData?.detail || 'Failed to confirm MPESA purchase';
      showToast(errMsg, 'error');
      setIsLoading(false);
      return;
    }
    ...
    showToast(err?.message || 'An error occurred during MPESA confirmation', { type: 'error' });
  } finally {
    setIsLoading(false);
  }
};
...
<table className="mt-10 min-w-full bg-white border border-gray-200 shadow-sm rounded-xl overflow-hidden">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="text-left px-4 py-2">Amoun Used</th>
          ...
          <tbody>
            {isFetching ? (
              ...
              transactions.map((tx, i) => (
                <tr key={i} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2">
                    ...
                    >
                      {tx.sct_tx_hash.slice(0, 6)}...{tx.sct_tx_hash.slice(-4)}
                    </a>
                  </td>
			  ...
			  {tx.payment_method === 'STRK' ? (
			    ...
			    >
			      {tx.payment_tx_id.slice(0, 6)}...{tx.payment_tx_id.slice(-4)}
			    </a>
			  ) : (
  ...
  <div className="sm:hidden">
    {transactions.map((tx, i) => (
      <div key={i} className="border border-gray-200 rounded-lg shadow-sm p-4 mb-4 bg-white">
        <div className="flex justify-between items-center">
          <div>
            ...
            <div className="text-lg font-semibold">{tx.amount_sct} STC</div>
          </div>
          <button
...
{/* Modal */}
{/* Modal */}
<Dialog open={!!selectedTx} onClose={() => setSelectedTx(null)} className="relative z-50">
  <div className="fixed inset-0 bg-black/30" aria-hidden="true" />
  <div className="fixed inset-0 flex items-center justify-center p-4">
    <Dialog.Panel className="bg-white rounded-lg p-6 max-w-md w-full shadow-xl">
      <Dialog.Title className="text-lg font-bold mb-4">Transaction Details</Dialog.Title>
      <ul className="space-y-2 text-sm text-gray-700">
        <li><strong>Date:</strong> {new Date(selectedTx?.date).toLocaleString()}</li>
        <li><strong>Amount (SCT):</strong> {selectedTx?.amount_sct}</li>
        <li><strong>Status:</strong> {selectedTx?.status || 'Confirmed'}</li>

        {/* STRK Used or Ksh */}
        {selectedTx?.payment_method === 'STRK' ? (
          <li><strong>STRK Used:</strong> {selectedTx?.strk_used}</li>
        ) : (
          <li><strong>Ksh:</strong> {selectedTx?.strk_used}</li>
        )}

        <li><strong>SCT Tx Hash:</strong>{' '}
          <a
        ...
        <li><strong>Payment Tx:</strong>{' '}
          {selectedTx?.payment_method === 'STRK' ? (
            ...
            >
              {selectedTx?.payment_tx_id.slice(0, 6)}...{selectedTx?.payment_tx_id.slice(-4)}
            </a>
          ) : (
            <span className="break-all">
              {selectedTx?.payment_tx_id.slice(0, 6)}...{selectedTx?.payment_tx_id.slice(-4)}
        ...
        <li><strong>Payment Method:</strong> {selectedTx?.payment_method}</li>
      </ul>
      <button
